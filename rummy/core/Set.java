...
public class Set implements SetInterface, Comparable<Set> {
  ...
  /**
   * Returns trie if the set is valid or not (i.e. contains at least 3 cards and no
   * more than 4 cards).
   * 
  ...
  public boolean isValid() {
    boolean threeCards = hand.size() <= CAPACITY;
    boolean fourCards = hand.size() >= MIN_NUM_NECESSARY_TO_FORM_SET;
    return threeCards && fourCards;
  }

  /**
   * Gets cards selected to form a set.
   * 
   * @return arr containing the cards for the Set.
   */
   ...
   * 
   * @return char of rank as defined in CardInterface.
   */
  public char getRank() {
    return rank;
  }
