 ...
 * class will provide the definition of what constitutes a hand for the game
...
public class Hand implements HandInterface, Comparable<Hand> {
   ...
   * 
   * @param index = the position of the Card that needs to be accessed.
   * @return the Card of interest, or null if not found.
   */
  public Card getCard(int index) {
    if (index < 0 || index >= getNumberOfCards()) return null;
   ...
   * @return position index of card if found, or -1 if not found.
   */
   ...
   * the hand.
   * 
   * @return returns Card[] of Cards found in deck or null if not
   *         found.
   */
  public Card[] findSet() {

    if (hand.size() < Set.MIN_NUM_NECESSARY_TO_FORM_SET) return null;
    ...
    for (int i = 0; i < hand.size(); i++) {
      ...
      if (i + 3 < hand.size()) set.addCard(hand.get(i + 3));
      
      if (set.isValid()) return set.getCards();
   ...
   * their rank (ace = 1, two = 2, etc.) in points with face cards
